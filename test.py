    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            for b in range(g3.size(0)):
                ...
                if not os.path.exists(f'./test/CH_SIMS_50_BATCH_SIZE/spec_mel/{sub_name}'):
                    os.makedirs(f'./test/CH_SIMS_50_BATCH_SIZE/spec_mel/{sub_name}')
                np.savez(f'./test/CH_SIMS_50_BATCH_SIZE/spec_mel/{sub_name}/{file_name}.npz',
                         mel=g3[b, :, :, :mel_len[b]].detach().cpu().numpy(),
                         spec=gs[b, :, :, :mel_len[b]].detach().cpu().numpy())

                if not os.path.exists(f'./test/CH_SIMS_50_BATCH_SIZE/wav/{sub_name}'):
                    os.makedirs(f'./test/CH_SIMS_50_BATCH_SIZE/wav/{sub_name}')
                sf.write(f'./test//CH_SIMS_50_BATCH_SIZE/wav/{sub_name}/{file_name}.wav', wav_spec[b], 16000, subtype='PCM_16')
        ...
        with open(f'./test/GRID_TEST/metric.txt', 'w') as f:
