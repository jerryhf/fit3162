...
class MultiDataset(Dataset):
        ...
        if subject == 'overlap':
            if mode == 'train':
                with open('./data/train.txt', 'r') as f:
                    lines = f.readlines()
                for l in lines:
                    file_list.append(os.path.join(grid, l.strip()))
            else:
            ...
            if mode == 'train':
                ...
                with open('./data/test.txt', 'r') as f:
                    lines = f.readlines()
                for l in lines:
                    file_list.append(os.path.join(grid, l))
    ...
    def __getitem__(self, idx):
        file_path = self.file_paths[idx].strip()
        
        vid, _, info = torchvision.io.read_video(file_path, pts_unit='sec')
    
        # Create the audio file path based on the video file path
        # Replace only the last part of 'video' with 'audio'
        audio_file_path = file_path.rsplit('video', 1)[0] + "audio" + file_path.rsplit('video', 1)[1]
        audio_file_path = audio_file_path[:-4] + '.flac'
        audio, info['audio_fps'] = librosa.load(audio_file_path, sr=16000)
