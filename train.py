...
from src.data.vid_aud_chsims import MultiDataset
...
def parse_args():
    ...
    parser.add_argument("--checkpoint_dir", type=str, default='./data/checkpoints/CH-SIMS')
...
def train(v_front, gen, post, dis1, dis2, dis3, s_dis, train_data, epochs, optimizer, scheduler, args):
    ...
    for epoch in range(args.start_epoch, epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            print("Running v_front...")
            phon, sent = v_front(vid.cuda())  # B,S,512, B,512
            print("v_front completed.")
            ...
            print(f"step = {step}, eval_step = {args.eval_step}")
            print(f"save condition = {step % args.eval_step}. Only saves if 0")
            if step % args.eval_step == 0:
                ...
                print(f'Current logs[1]: {logs[1]}, Best val_stoi: {best_val_stoi}')
                if logs[1] > best_val_stoi:
                    ...
                    'dis3_state_dict': dis3_state_dict, 's_dis_state_dict': s_dis_state_dict},
                    os.path.join(args.checkpoint_dir,
                        f'Epoch_{epoch:04d}_stoi_{logs[1]:.3f}_estoi_{logs[2]:.3f}_pesq_{logs[3]:.3f}_genloss_{gen_loss.cpu().item():.3f}.ckpt'))

                    print(f"Checkpoint saved at {args.checkpoint_dir}.")
                else:
                  print('Not saving this checkpoint as it does not improve VAL_stoi.')
        ...
        epoch_time_taken = epoch_end_time - epoch_start_time  # Calculate time taken for the epoch

        print(f"Time taken for epoch {epoch}: {epoch_time_taken:.2f} seconds")    
